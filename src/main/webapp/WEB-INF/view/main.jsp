...
<head>
    ...
    <script type="text/javascript">
        ...
        function submitPwdChange() {
            ...
            if(oldpwd==""){
                $.messager.alert("提示","原始密码为必填项");
                return;
            }if(newpwd==""){
                $.messager.alert("提示","新密码为必填项");
                return;
            }if(repwd==""){
                $.messager.alert("提示","确认密码为必填项");
                return;
            }
            if(repwd!=newpwd){
                $.messager.alert("提示","俩次密码输入不一致");
                return;
            }
            $.ajax({
                ...
                success:function (result) {
                   if (result.success){
                       $.messager.alert("提示","修改成功！请立即重新登陆","",function () {
                           window.location.replace(path+"/user/logout");
                       });
...
<div data-options="region:'north',border:false"
    ...
    <div style="float:right;color: #FFFFFF;padding-top: 10px">
        <strong>
            <a style="color: #FFFFFF;text-decoration: none" href="javascript:void(0)" onclick="openPwdChangeWin()">
                欢迎您${sessionScope.username}
            </a>
            &nbsp;&nbsp;&nbsp;&nbsp;
            <a style="color: #FFFFFF;text-decoration: none" href="javascript:void(0)" onclick="logout()">退出登录</a>
        </strong>
    </div>
</div>
...
<div id="pwdChangeWin" style="width:100%;padding:10px 30px;" align="center">
